...
function generateRounds(count: number, challengeFlagIds?: string[]): RoundData[] {
  ...
  // Rotate the eligible pool by least-recently-shown so players cycle through
  // every country with neighbors before seeing a repeat.
  const shuffled = rotateByLeastRecentlyShown(eligible, (id) => id).slice(0, count);
  const rounds: RoundData[] = [];
